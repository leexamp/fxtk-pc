...
static void on_keys_view(fx_widget_t *w, void *ud) {
     ...
     * 头部标题条 + 两张状态卡(Keyboard/Mouse) + 最近事件日志(交替行色) + 画布内指针十字标。 */
    ...
    fx_draw_text_c(20, 14, "Input monitor · keyboard / mouse / wheel", FX_WHITE, FX_RGB(33, 150, 243));
    ...
    if (s_have && (k.key != s_prev_key.key || k.utf8[0] != s_prev_key.utf8[0])) {
        if (k.utf8[0]) snprintf(ev, sizeof(ev), "key   「%s」", k.utf8);
        else if (k.key == FX_KEY_BACKSPACE) snprintf(ev, sizeof(ev), "key   Backspace");
        else if (k.key == FX_KEY_RETURN)    snprintf(ev, sizeof(ev), "key   Enter");
        else if (k.key == FX_KEY_ESCAPE)    snprintf(ev, sizeof(ev), "key   Esc");
        else if (k.key == FX_KEY_LEFT)      snprintf(ev, sizeof(ev), "key   ←");
        else if (k.key == FX_KEY_RIGHT)     snprintf(ev, sizeof(ev), "key   →");
        else                                snprintf(ev, sizeof(ev), "key   code=%d", (int)k.key);
        dirty = 1;
    } else if (mp != s_prev_mp) {
        snprintf(ev, sizeof(ev), "Mouse  %s", mp ? "down" : "up");
        dirty = 1;
    } else if (mp && (mx != s_prev_mx || my != s_prev_my)) {
        snprintf(ev, sizeof(ev), "drag to (%d, %d)", mx, my);
        dirty = 1;
    }
    if (dirty) {
    ...
    /* 状态卡 */
    int cardw = (cw - 34) / 2;
    const char *ttl[2] = { "Keyboard", "Mouse" };
    char val[2][64];
    if (k.utf8[0] || k.key) snprintf(val[0], sizeof(val[0]), "%s%s", k.utf8[0] ? "char " : "code ", k.utf8[0] ? k.utf8 : "");
    else snprintf(val[0], sizeof(val[0]), "waiting…");
    if (k.utf8[0] == 0 && k.key) snprintf(val[0], sizeof(val[0]), "code %d", (int)k.key);
    snprintf(val[1], sizeof(val[1]), "%d, %d  ·  %s", mx, my, mp ? "down" : "up");
    for (int i = 0; i < 2; i++) {
    ...
    if (rows > 0) {
        ...
        fxtk_draw_text_size(13, 20, ly0, "Event log (last 18)", FX_RGB(140, 145, 160), CARD);
    ...
    fxtk_draw_text_size(13, 20, ch - 22,
        "提示: 移动Mouse看画布内十字标 · 点击任意按钮看状态变化 · 滚轮滚动会记录在事件日志里",
        FX_RGB(110, 115, 130), BG);

    /* 画布内指针十字标(让"Mouse位置"这件事看得见) */
