...
## 设计取向：嵌入式级的资源纪律 + PC 级的渲染能力

核心里没有"跟着平台一起长胖"的部分，这套取向是有意保持的：

- **静态优先**：控件、滚动状态、扩展控件都有**固定池**（`FX_MAX_WIDGETS` / `FX_MAX_SCROLL_STATES` / `FX_MAX_EXTRA_WIDGETS`，
  均可用编译宏放大），热路径不做动态分配 → 行为可预测、无碎片、崩溃面小。
- **零平台强依赖**：核心只依赖 `fx_driver_t` 契约与自带服务层；PC 端默认接 **sokol(OpenGL)**，
  ESP32 端走同一套核心（`tools/esp32_smoke.sh` 在 CI 里做接口级编译冒烟，无需 ESP-IDF 工具链）。
  日志统一走 `fx_log()`（ESP32 侧自动转发到 `esp_log_write`），核心不包含任何平台日志头。
- **体积纳入验收**：Linux 单文件 **361KB**、Windows 单 exe **433KB**，均**无第三方动态库**（只有系统 OpenGL/DLL）。
- **性能实测（无头、无 vsync，本机）**：1920×1080 压测页饱和 **2819 控件 ≈ 9.0~9.3ms/帧（107~111fps）**；
  同一台机器同一命令下，v2.3 为 9.40~9.95ms（100~106fps）。桌面窗口下受合成器限制通常仍是 60Hz，
  所以真实上限要用无头基准 `make bench` 来看。

换句话说：**用嵌入式的内存纪律写 PC GUI**，换来的是"小、快、可预测"——也正是它能在 400KB 级体积里
跑出 GPU 加速(两层抗锯齿、真透视形变、实时光线步进)的原因。
