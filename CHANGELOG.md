...
### 测试（v2.4.4 B7：补上项目里第一批颜色契约断言）
评审指出"每个测试都通过，但零像素断言"——核实属实：假驱动的 `fill_rect` 把颜色参数 `(void)c` 丢掉、
`push_pixels` 是空实现，因此全部检查只看面积与计数。
- 假驱动现在记录**最后一次 `fill_rect` 的矩形与颜色**，并累计 `push_pixels` 收到的像素数。
- 新增三条断言：填充矩形是否**按原样**送达驱动、颜色是否**按原样**送达、以及离屏画布路径的推送计数。
  实测输出：`[ok] 填充矩形按原样送达驱动 (11,22)-(33,44)`、`[ok] 填充颜色按原样送达驱动 (0x112233)`、
  `[info] 全程 push_pixels 累计像素 = 124`（>0 说明离屏画布确实推送了像素）。
- 同步更正 README 与测试文件头部的覆盖说明（此前写着"没有任何像素断言"，已过期）。
- **仍未做**：逐像素帧缓冲比对（把 push_pixels 写进内存帧缓冲再做像素级断言）、以及把金图纳入 CI。

### 审计整改（v2.4.4：B 级 —— CI 验对路径 / FXTK_AA 对任何应用生效 / bench 口径与文档一致）
- **B6 CI 验的路径已改对**：`windows-cross` 原先跑 `build_win_cross.sh` —— 那是**已废弃的 SDL 交叉路径**，
  产物是 exe + 三个 SDL DLL；而真正发布的是 `build_win_sokol.sh`（单 exe，`objdump` 验证只依赖系统 DLL）。
  现在 CI 改验发布路径，并顺带用 `--autotrim` 验证控件裁剪在 Windows 路径也成立。
- **B8 `FXTK_AA` 对任何应用生效**：文档（`docs/api.md`、`docs/guide.md` 及英文版）一直教用户设 `FXTK_AA=0|1|2`，
  但此前全仓库只有 `demo-main/app.c` 读它 → 用户自己的应用（含 `your_app`）照做无效。
  现在由框架 `fx_init()` 读取，显式调用 `fx_set_widget_aa()` 仍然优先。
- **B9 `make bench` 口径与文档一致**：原先跑 `1200 帧 / 480x272`（只到 84 个控件），而 README 引用的是
  "1080P / 2820 控件 / 102–130 fps" → 现在 `make bench` 默认就是这个口径，另加 `make bench-fast` 供快速冒烟。
  注：实测受机器负载影响明显（同一口径在空闲时 9.5–9.8 ms，在连续编译后的高负载下可到 12.1 ms / 82.7 fps），
  引用数字时请以区间与当时负载为准。
- **B2 无需改动**：告警文案建议"加大 `FX_MAX_SCROLL_STATES`" —— A2 已把循环从硬编码 8 改为该宏，建议现已成立。

### 审计整改（v2.4.4：按第三方深度评审逐条核实与修正）
评审给出「行号 + 复现路径 + 证据等级」，我们逐条自行核对，**全部属实**。已完成 P0 与关键表述更正：
...
- **★ 二进制入库（已修）**：`your_app/` 的 exe 与 `.so` 曾被 tracked，而 `package_release.sh` 用 `git archive` 打「纯源码」包
  → 二进制会随源码包出门。已 `git rm --cached` 并补 `.gitignore`。
- **★ 对外表述更正（重要）**：README 曾把「脏区合并重绘」列为头号性能卖点，**该功能当前是死代码**
  （`fx_repaint_rect()` 开头无条件 `s_full=1`，消费端 `else if (s_full)` 排在脏区分支之前，合并结果永远用不到）。
  现状是**每次请求都整帧重绘**（因此不残影，但也没有脏区优化）。README 已改为如实描述，代码注释里写明启用条件与验证方法。

### 轻量化（v2.4.3：控件池收窄 + sokol 体积结论落定）
- **控件池 PC 默认 16384 → 8192**：`.bss` **16.6MB → 14.1MB**（省约 2.5MB 常驻内存）；
  压测页实际只到约 2820（受画布容量限制），因此**可达规模与帧率都不受影响**（复核：2820 控件 8.701 ms / 114.9 fps）。
- **sokol 校验层**：确认它由 `#if defined(SOKOL_DEBUG)` 控制，而驱动**从未定义**它 ⇒ **校验层本来就是关闭的**，
  所以此前试的 `-DNDEBUG` 省 0KB 是必然的（不是失败，而是本来就没开）。**体积这条路到此为止**：
  359KB text = sokol_app + sokol_gfx + GL 加载器 + 框架 + 12 页演示，想再小只能砍功能（用户已明确不要）。
- 顺带确认：本仓库 vendored 的 sokol 是 **X11-only** 版本，没有可裁剪的 Wayland 分支。
