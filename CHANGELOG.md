...
### 结论（v2.4.3：压测页"每帧强制整画布重绘"不是缺陷，是必需的）
之前把这一句记为"性能与闪烁的根源"，实测后**这个判断是错的**，更正如下：
- **去掉它**（临时实验）：帧时间 6.33 → 6.59 ms（噪声范围内，**没有变快**），但画面实色像素 **27609 → 18795**，
  即约**三分之一控件没被画出来** → 说明它**必须有**：画布局部重绘路径会把"移动后的新位置"裁掉，
  这句是让画面完整的必要手段。
- **为什么它也省不掉**：压测页每帧移动的是**全部**约 1200–2800 个控件，脏区并集本就是整个画布 →
  即使不写这一句，实际要重绘的范围也已经是全画布。所以"1080P 下 sokol 路径 59fps"是**这一压力模型固有的**，
  不是框架低效：每帧必须完整重绘并上传整块画布。
- 因此**第 4 项关闭**（非缺陷）。真要在这类页面上冲更高帧率，只能改压力模型本身
  （少动一些控件、或让画布内容走 GPU 绘制），那是新功能而不是修 bug。
