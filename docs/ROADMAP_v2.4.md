...
| P5 控件审美 + 设计令牌 | ⏳ 8/9(按钮/输入框/滑杆/标签页/列表/进度条/复选框/卡片 ✅) | `fxtk_tokens.h` 已落地(纯重构, 金图 20/20 不变); 按钮轮完成(圆角 7/描边 25%/按下整块压暗, 零额外绘制调用); 流程见 docs/design/README.md; 余: 标签(label) 最后一项 |
