...
1. **默认只喂 `TOKENS.md`（约 900 token）**。不要一次喂 `REFERENCE.md` + `ref_app.c`
...
| 文件 | 大小 | 什么时候用 |
|---|---|---|
| `TOKENS.md` | ~900 token | **默认喂这个**。API 速查卡，够写绝大多数小程序 |
| `calc.c` | ~890 token | 完整可运行的计算器（数据驱动 + 求值逻辑，逻辑已单测通过） |
| `ref_app.c` | ~1400 token | 参考实现：每种控件的标准用法 |
| `REFERENCE.md` | ~3000 token | 完整文档 + 陷阱表 + 属性宏全清单。**按需查，别全量喂** |
...
6. 改控件用 `fx_set_title`（不是 `fx_set_text`），改完自动重绘，**不要调 `fx_repaint`**。
